...
#define DECLARE_HAS_MEMBER(__trait_name__, __member_name__)                    \
                                                                               \
  template <typename __boost_has_member_T__> class __trait_name__ {            \
...
struct FringeScaleRef {
  ...
  template <typename T>
  static float &
  Get(typename std::enable_if<HasFringeScale<T>::value, T>::type *drawList) {
  ...
  template <typename T>
  static float &
  Get(typename std::enable_if<!HasFringeScale<T>::value, T>::type *) {
...
template <typename T, typename Id = typename T::IdType> struct ObjectWrapper {
  Id m_ID;
  T *m_Object;
...
struct Object {
  ...
  Object(EditorContext *editor)
      : Editor(editor), m_IsLive(true), m_IsSelected(false),
...
struct Pin final : Object {
  ...
  Pin(EditorContext *editor, PinId id, PinKind kind)
      : Object(editor), m_ID(id), m_Kind(kind), m_Node(nullptr), m_Bounds(),
        m_PreviousPin(nullptr), m_Color(IM_COL32_WHITE),
        m_BorderColor(IM_COL32_BLACK), m_BorderWidth(0), m_Rounding(0),
        m_Corners(0), m_Dir(0, 0), m_Strength(0), m_Radius(0), m_ArrowSize(0),
        m_ArrowWidth(0), m_SnapLinkToDir(true), m_HasConnection(false),
...
struct Node final : Object {
  ...
  Node(EditorContext *editor, NodeId id)
      : Object(editor), m_ID(id), m_Type(NodeType::Node), m_Bounds(),
        m_ZPosition(0.0f), m_Channel(0), m_LastPin(nullptr), m_DragStart(),
        m_Color(IM_COL32_WHITE), m_BorderColor(IM_COL32_BLACK),
        m_BorderWidth(0), m_Rounding(0), m_GroupBounds(),
        m_HighlightConnectedLinks(false), m_RestoreState(false),
...
struct Link final : Object {
  ...
  Link(EditorContext *editor, LinkId id)
      : Object(editor), m_ID(id), m_StartPin(nullptr), m_EndPin(nullptr),
        m_Color(IM_COL32_WHITE), m_Thickness(1.0f) {}
...
struct NodeSettings {
  ...
  NodeSettings(NodeId id)
      : m_ID(id), m_Location(0, 0), m_Size(0, 0), m_GroupSize(0, 0),
        m_WasUsed(false), m_Saved(false), m_IsDirty(false),
...
struct Settings {
  ...
  Settings()
      : m_IsDirty(false), m_DirtyReason(SaveReasonFlags::None),
        m_ViewScroll(0, 0), m_ViewZoom(1.0f), m_VisibleRect() {}
...
struct Control {
  ...
  Control(Object *hotObject, Object *activeObject, Object *clickedObject,
          ...
          bool backgroundActive, int backgroundClickButtonIndex,
          int backgroundDoubleClickButtonIndex)
      : HotObject(hotObject), ActiveObject(activeObject),
        ClickedObject(clickedObject), DoubleClickedObject(doubleClickedObject),
        HotNode(nullptr), ActiveNode(nullptr), ClickedNode(nullptr),
        DoubleClickedNode(nullptr), HotPin(nullptr), ActivePin(nullptr),
        ClickedPin(nullptr), DoubleClickedPin(nullptr), HotLink(nullptr),
        ActiveLink(nullptr), ClickedLink(nullptr), DoubleClickedLink(nullptr),
        BackgroundHot(backgroundHot), BackgroundActive(backgroundActive),
...
struct NavigateAction final : EditorAction {
  ...
  void NavigateTo(const ImRect &bounds, ZoomMode zoomMode,
                  float duration = -1.0f,
...
struct EditorContext {
  ...
  void FindLinksForNode(NodeId nodeId, vector<Link *> &result,
                        bool add = false);
  ...
  template <typename T> ImRect GetBounds(const std::vector<T *> &objects) {
