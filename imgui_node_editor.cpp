...
ed::EditorContext::EditorContext(const ax::NodeEditor::Config *config)
    : m_Config(config), m_EditorActiveId(0), m_IsFirstFrame(true),
      m_IsFocused(false), m_IsHovered(false), m_IsHoveredWithoutOverlapp(false),
      m_ShortcutsEnabled(true), m_Style(), m_Nodes(), m_Pins(), m_Links(),
      m_SelectionId(1), m_LastActiveLink(nullptr), m_Canvas(),
      m_IsCanvasVisible(false), m_NodeBuilder(this), m_HintBuilder(this),
      m_CurrentAction(nullptr), m_NavigateAction(this, m_Canvas),
      m_SizeAction(this), m_DragAction(this), m_SelectAction(this),
      m_ContextMenuAction(this), m_ShortcutAction(this),
      m_CreateItemAction(this), m_DeleteItemsAction(this),
      m_AnimationControllers{&m_FlowAnimationController},
      m_FlowAnimationController(this), m_HoveredNode(0), m_HoveredPin(0),
      m_HoveredLink(0), m_DoubleClickedNode(0), m_DoubleClickedPin(0),
      m_DoubleClickedLink(0), m_BackgroundClickButtonIndex(-1),
      m_BackgroundDoubleClickButtonIndex(-1), m_IsInitialized(false),
      m_Settings(), m_DrawList(nullptr), m_ExternalChannel(0) {}
...
void ed::EditorContext::Begin(const char *id, const ImVec2 &size) {
  ...
  if (!m_IsInitialized) {
    // Cycle canvas, so it has a chance to initialize its size before settings
    // are loaded
  ...
  if (!ImRect_IsEmpty(previousVisibleRect) && previousSize != canvasSize) {
    ...
    m_NavigateAction.NavigateTo(previousVisibleRect,
                                Detail::NavigateAction::ZoomMode::Exact, 0.0f);
  }
...
void ed::EditorContext::End() {
  ...
  // node drawing order.
  {
    ...
    ImDrawList_ChannelsGrow(m_DrawList, m_DrawList->_Splitter._Count +
    ...
    auto copyNode = [this, &targetChannel](Node *node) {
      ...
      for (int i = 0; i < c_ChannelsPerNode; ++i)
        ImDrawList_SwapChannels(m_DrawList, node->m_Channel + i,
                                targetChannel + i);
    ...
    for (int i = 0; i < c_LinkChannelCount; ++i, ++targetChannel)
      ImDrawList_SwapChannels(m_DrawList, c_LinkStartChannel + i,
                              targetChannel);
  ...
  // Move user and hint channels to top
  {
    ...
    ImDrawList_SwapChannels(m_DrawList, c_UserChannel_HintsBackground,
                            channelCount + 0);
    ImDrawList_SwapChannels(m_DrawList, c_UserChannel_Hints, channelCount + 1);
    ImDrawList_SwapChannels(m_DrawList, c_UserChannel_Content,
                            channelCount + 2);
  ...
  // Draw border
  {
    ...
    m_DrawList->AddRect(m_Canvas.Rect().Min + ImVec2(1, 1),
                        m_Canvas.Rect().Max - ImVec2(1, 1),
                        ImColor(borderShadoColor));
    m_DrawList->AddRect(m_Canvas.Rect().Min, m_Canvas.Rect().Max,
                        ImColor(borderColor));
  }
...
void ed::EditorContext::Suspend(SuspendFlags flags) {
  IM_ASSERT(m_DrawList != nullptr &&
            "Suspend was called outiside of Begin/End.");
...
void ed::EditorContext::Resume(SuspendFlags flags) {
  IM_ASSERT(m_DrawList != nullptr &&
            "Reasume was called outiside of Begin/End.");
  if ((flags & SuspendFlags::KeepSplitter) != SuspendFlags::KeepSplitter)
    ImDrawList_SwapSplitter(m_DrawList, m_Splitter);
  auto lastChannel = m_DrawList->_Splitter._Current;
  m_DrawList->ChannelsSetCurrent(m_ExternalChannel);
...
bool ed::EditorContext::IsFocused() { return m_IsFocused; }

bool ed::EditorContext::IsHovered() const { return m_IsHovered; }
...
int ed::EditorContext::CountLiveNodes() const {
  return (int)std::count_if(m_Nodes.begin(), m_Nodes.end(),
                            [](const Node *node) { return node->m_IsLive; });
}

int ed::EditorContext::CountLivePins() const {
  return (int)std::count_if(m_Pins.begin(), m_Pins.end(),
                            [](const Pin *pin) { return pin->m_IsLive; });
}

int ed::EditorContext::CountLiveLinks() const {
  return (int)std::count_if(m_Links.begin(), m_Links.end(),
...
void ed::EditorContext::LoadSettings() {
  ...
  if (ImRect_IsEmpty(m_Settings.m_VisibleRect)) {
    ...
    m_NavigateAction.NavigateTo(m_Settings.m_VisibleRect,
                                NavigateAction::ZoomMode::Exact, 0.0f);
  }
}
...
ed::Control ed::EditorContext::BuildControl(bool allowOffscreen) {
  ...
  auto emitInteractiveArea = [&emitInteractiveAreaEx,
                              extraFlags](ObjectId id, const ImRect &rect) {
  ...
  return Control(hotObject, activeObject, clickedObject, doubleClickedObject,
                 isBackgroundHot, isBackgroundActive, backgroundClickButonIndex,
...
void ed::EditorContext::ShowMetrics(const Control &control) {
  ...
  ImGui::Text("Is Hovered (without overlapp): %s",
  ...
  ImGui::Text("Hot Object: %s (%p)", getHotObjectName(),
              control.HotObject ? control.HotObject->ID().AsPointer()
                                : nullptr);
...
bool ed::Settings::Parse(const std::string &string, Settings &settings) {
  ...
  if (viewValue.is_object()) {
    ...
    if (!viewValue.contains("visible_rect") ||
        !tryParseVector(viewValue["visible_rect"]["min"],
                        result.m_VisibleRect.Min) ||
        !tryParseVector(viewValue["visible_rect"]["max"],
...
ed::FlowAnimation::FlowAnimation(FlowAnimationController *controller)
    : Animation(controller->Editor), Controller(controller), m_Link(nullptr),
      m_Offset(0.0f), m_PathLength(0.0f) {}
...
void ed::FlowAnimationController::Flow(Link *link, FlowDirection direction) {
  ...
  animation->Flow(link, editorStyle.FlowMarkerDistance,
...
ed::NavigateAction::NavigateAction(EditorContext *editor,
    ...
    : EditorAction(editor), m_IsActive(false), m_Zoom(1), m_VisibleRect(),
      m_Scroll(0, 0), m_ScrollStart(0, 0), m_ScrollDelta(0, 0),
      m_Canvas(canvas), m_WindowScreenPos(0, 0), m_WindowScreenSize(0, 0),
      m_Animation(editor, *this), m_Reason(NavigationReason::Unknown),
      m_LastSelectionId(0), m_LastObject(nullptr), m_MovingOverEdge(false),
...
ed::EditorAction::AcceptResult
ed::NavigateAction::Accept(const Control &control) {
  ...
  if (Editor->CanAcceptUserInput() && ImGui::IsKeyPressed(GetKeyIndexForF()) &&
    ...
    if (!Editor->GetSelectedObjects().empty()) {
      if (m_Reason != NavigationReason::Selection ||
        ...
        m_LastSelectionId = Editor->GetSelectionId();
        NavigateTo(Editor->GetSelectionBounds(), zoomMode, -1.0f,
    ...
    if (navigateToContent)
      NavigateTo(Editor->GetContentBounds(),
                 NavigateAction::ZoomMode::WithMargin, -1.0f,
...
void ed::NavigateAction::ShowMetrics() {
  ...
  ImGui::Text("    Visible Rect: { l=%g t=%g, r=%g b=%g w=%g h=%g }",
              m_VisibleRect.Min.x, m_VisibleRect.Min.y, m_VisibleRect.Max.x,
              m_VisibleRect.Max.y, m_VisibleRect.Max.x - m_VisibleRect.Min.x,
              m_VisibleRect.Max.y - m_VisibleRect.Min.y);
}

void ed::NavigateAction::NavigateTo(const ImRect &bounds, ZoomMode zoomMode,
...
bool ed::NavigateAction::MoveOverEdge(const ImVec2 &canvasSize) {
  ...
  const auto screenRect = ImRect(ImGui::GetCursorScreenPos(),
...
ed::SizeAction::SizeAction(EditorContext *editor)
    : EditorAction(editor), m_IsActive(false), m_Clean(false),
      m_SizedNode(nullptr), m_Pivot(NodeRegion::None),
...
bool ed::SizeAction::Process(const Control &control) {
  ...
  if (control.ActiveNode == m_SizedNode) {
    ...
    auto minimumSize = ImMax(m_MinimumSize, m_StartBounds.GetSize() -
...
ed::DragAction::DragAction(EditorContext *editor)
    : EditorAction(editor), m_IsActive(false), m_Clear(false),
...
ed::SelectAction::SelectAction(EditorContext *editor)
    : EditorAction(editor), m_IsActive(false), m_SelectGroups(false),
      m_SelectLinkMode(false), m_CommitSelection(false), m_StartPoint(),
      m_Animation(editor) {}

ed::EditorAction::AcceptResult
ed::SelectAction::Accept(const Control &control) {
  IM_ASSERT(!m_IsActive);
  ...
  } else if (control.BackgroundClickButtonIndex ==
...
ed::ContextMenuAction::ContextMenuAction(EditorContext *editor)
    : EditorAction(editor), m_CandidateMenu(Menu::None),
      m_CurrentMenu(Menu::None), m_ContextId() {}

ed::EditorAction::AcceptResult
ed::ContextMenuAction::Accept(const Control &control) {
...
ed::ShortcutAction::ShortcutAction(EditorContext *editor)
    : EditorAction(editor), m_IsActive(false), m_InAction(false),
      m_CurrentAction(Action::None), m_Context() {}

ed::EditorAction::AcceptResult
ed::ShortcutAction::Accept(const Control &control) {
  ...
  if (candidateAction != None) {
    if (candidateAction != Paste && candidateAction != CreateNode) {
      ...
      const auto hasAnyLinks =
          std::any_of(m_Context.begin(), m_Context.end(), [](Object *object) {
...
ed::CreateItemAction::CreateItemAction(EditorContext *editor)
    : EditorAction(editor), m_InActive(false), m_NextStage(None),
      m_CurrentStage(None), m_ItemType(NoItem), m_UserAction(Unknown),
      m_LinkColor(IM_COL32_WHITE), m_LinkThickness(1.0f), m_LinkStart(nullptr),
      m_LinkEnd(nullptr),
...
ed::EditorAction::AcceptResult
ed::CreateItemAction::Accept(const Control &control) {
  IM_ASSERT(!m_IsActive);
...
ed::DeleteItemsAction::DeleteItemsAction(EditorContext *editor)
    : EditorAction(editor), m_IsActive(false), m_InInteraction(false),
      m_CurrentItemType(Unknown), m_UserAction(Undetermined) {}
...
void ed::DeleteItemsAction::DeleteDeadLinks(NodeId nodeId) {
  ...
  for (auto link : links) {
    ...
    auto it =
        std::find(m_CandidateObjects.begin(), m_CandidateObjects.end(), link);
...
ed::EditorAction::AcceptResult
ed::DeleteItemsAction::Accept(const Control &control) {
  IM_ASSERT(!m_IsActive);
...
void ed::NodeBuilder::End() {
  ...
  if (editorStyle.NodePadding.x != 0 || editorStyle.NodePadding.y != 0 ||
    ...
    ImGui::Dummy(ImVec2(
        0, 0)); // bump cursor at the end of the line and move to next one
