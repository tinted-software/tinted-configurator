...
struct VtxCurrentOffsetRef {
  ...
  template <typename T>
  static unsigned int &
  Get(typename std::enable_if<HasVtxCurrentOffset<T>::value, T>::type
          *drawList) {
...
void ImGuiEx::Canvas::End() {
  ...
  // m_DrawList->AddRect(m_WidgetPosition - ImVec2(1.0f, 1.0f), m_WidgetPosition
  // + m_WidgetSize + ImVec2(1.0f, 1.0f), IM_COL32(196, 0, 0, 255));
...
ImGuiEx::CanvasView
ImGuiEx::Canvas::CalcCenterView(const ImVec2 &canvasPoint) const {
...
ImGuiEx::CanvasView
ImGuiEx::Canvas::CalcCenterView(const ImRect &canvasRect) const {
...
void ImGuiEx::Canvas::Resume() {
  ...
  // Check: Number of calls to Resume() do not match calls to Suspend(). Please
  // check your code.
  IM_ASSERT(m_SuspendCounter > 0);
  if (--m_SuspendCounter == 0)
    EnterLocalSpace();
...
ImVec2 ImGuiEx::Canvas::FromLocal(const ImVec2 &point,
...
ImVec2 ImGuiEx::Canvas::FromLocalV(const ImVec2 &vector,
                                   const CanvasView &view) const {
...
ImVec2 ImGuiEx::Canvas::ToLocal(const ImVec2 &point,
                                const CanvasView &view) const {
...
ImVec2 ImGuiEx::Canvas::ToLocalV(const ImVec2 &vector,
                                 const CanvasView &view) const {
...
void ImGuiEx::Canvas::LeaveLocalSpace() {
  ...
  if (m_View.Scale != 1.0f) {
    ...
    for (int i = m_DrawListFirstCommandIndex; i < m_DrawList->CmdBuffer.size();
         ++i) {
  ...
  if (m_DrawListCommadBufferSize > 0) {
    if (m_DrawList->CmdBuffer.size() > m_DrawListCommadBufferSize &&
      ...
      m_DrawList->CmdBuffer.erase(m_DrawList->CmdBuffer.Data +
